...
public class CylinderMeshBuilder implements MeshBuilder {
    ...
    // Added boolean argument: usePlanarMapping
    public static String buildMesh(CylinderShape cylinder) {
        ...
        for (int t = 0; t < segmentsPerTile.length; t++) {
            ...
            for (int k = 0; k < segs + 1; k++) {
                ...
                vertices.add(new Vector3f((float) vx, (float) offsetY, (float) vz));
                UVs.add(new Vector2f((float) u, 0.0f));
                normals.add(new Vector3f((float) nx, 0.0f, (float) nz));

                vertices.add(new Vector3f((float) vx, (float) (cylinder.size.y() + offsetY), (float) vz));
        ...
        vertices.add(new Vector3f(0.5f * cylinder.size.x() + offsetX, offsetY, 0.5f * cylinder.size.z() + offsetZ));
        ...
        for (int i = 0; i < cylinder.numSides; i++) {
            ...
            if (usePlanarMapping) {
                // Planar: Directly map the cosine/sine values to 0.0-1.0 range
                UVs.add(new Vector2f((float)(0.5 + 0.5 * Math.cos(angle)), (float)(0.5 + 0.5 * Math.sin(angle))));
            } else {
                UVs.add(diskToSquareUV((float) Math.cos(angle), (float) Math.sin(angle)));
            }
            normals.add(new Vector3f(0.0f, -1.0f, 0.0f));
        }
        ...
        vertices.add(new Vector3f(0.5f * cylinder.size.x() + offsetX, cylinder.size.y() + offsetY, 0.5f * cylinder.size.z() + offsetZ));
        UVs.add(new Vector2f(0.5f, 0.5f));
        normals.add(new Vector3f(0.0f, 1.0f, 0.0f));
        ...
        for (Vector3f vertex : vertices) builder.append(String.format("v %s %s %s\n", vertex.x(), vertex.y(), vertex.z()));
        for (Vector2f uv : UVs) builder.append(String.format("vt %s %s\n", uv.x(), uv.y()));
        for (Vector3f normal : normals) builder.append(String.format("vn %s %s %s\n", normal.x(), normal.y(), normal.z()));
